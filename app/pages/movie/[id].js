...
import { Image, useParam, useParams } from "blitz"
...
function Movie() {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/${id}?api_key=19bed41eecf289253751312a444bfc69&language=en-US&append_to_response=videos`
    )
        ...
        <div className="absolute inset-y-28 md:inset-y-auto md:bottom-10 inset-x-4 md:inset-x-12 space-y-6 z-50">
          ...
          <div className="flex items-center space-x-3 md:space-x-5">
            <button className="text-xs md:text-base bg-[#f9f9f9] text-black flex items-center justify-center py-2.5 px-6 rounded hover:bg-[#c6c6c6]">
              <Image src="/images/play-icon-black.svg" alt="" width={24} height={24} />
            ...
            <div className="rounded-full border-2 border-white flex items-center justify-center w-11 h-11 cursor-pointer bg-black/60">
              <PlusIcon className="h-6" width={40} height={40} />
            </div>

            <div className="rounded-full border-2 border-white flex items-center justify-center w-11 h-11 cursor-pointer bg-black/60">
              <Image src="/images/group-icon.svg" alt="" width={40} height={40} />
            </div>
          </div>
